    ...
    if (jassubObj.changed !== 0 || force) {
        ...
        if (asyncRender) {
            ...
            Promise.all(promises).then(bitmaps => {
                ...
                if (debug) {
                    times.JSBitmapGenerationTime = Date.now() - times.JSRenderTime
                }
...
const paintImages = ({ times, images, buffers }) => {
    ...
    if (offscreenRender) {
        ...
        for (const image of images) {
            if (image.image) {
                if (asyncRender) {
                    ...
                    bufferCtx.putImageData(new ImageData(
                        self.HEAPU8C.subarray(image.image, image.image + image.w * image.h * 4),
                        image.w,
                        image.h
                    ), 0, 0)
        ...
        if (offscreenRender === 'hybrid') {
            if (!images.length) {
                return postMessage(resultObject)
            }
            if (debug) {
...
self.canvas = ({ width, height, videoWidth, videoHeight, force }) => {
    ...
    self.height = height
    if (jassubObj) {
        jassubObj.resizeCanvas(width, height, videoWidth, videoHeight)
    }
    if (force) {
...
self.video = ({ currentTime, isPaused }) => {
    if (currentTime != null) {
